...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Product operation: kgcr history
    history = sub.add_parser("history", help="Show run history from the persistence store")
    history.add_argument(
        "--account", type=str, default=None, help="Filter run history by account ID"
    )
    history.add_argument(
        "--run-id", type=str, default=None, help="Fetch details for a specific run ID"
    )
    history.add_argument(
        "--db", type=str, default="kgcr.db", help="Path to SQLite database (default kgcr.db)"
    )
    history.add_argument(
        "--limit", type=int, default=50, help="Maximum number of runs to list (default 50)"
    )
    ...
    try:
        if args.run_id:
            ...
            if args.json:
                ...
                print(f"audience:       {run.audience}")
                print(f"terminal_state: {run.terminal_state}")
                print(f"account_id:     {run.account_id or 'none'}")
                print(f"created_at:     {run.created_at}")
            return 0

        runs = store.list_runs(account_id=args.account, limit=args.limit)
        if args.json:
            print(json.dumps([r.to_dict() for r in runs], indent=2, sort_keys=True))
            return 0

        print(f"Found {len(runs)} run record(s):")
        for r in runs:
            acc = f" (account: {r.account_id})" if r.account_id else ""
            line = (
                f"  [{r.created_at}] {r.run_id[:12]} | mode: {r.mode} | "
                f"state: {r.terminal_state}{acc}"
            )
            print(line)
        return 0
    finally:
