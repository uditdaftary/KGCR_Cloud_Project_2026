...
> **Status, 2026-07-30.**
> **Done:** the repo, the branch topology (`feature/udit` | `feature/manya` | `feature/tanmoy`),
> the tree restructure with all four gates green, and every required report section — abstract,
> 15-paper survey, Udit's research-gap analysis, objectives, novelty, both mandatory architecture
> diagrams, dataset details, and the AWS services planning table — in Markdown and generated
> `.docx`. `LICENSE` added.
> **Remaining:** `presentation/` deck; Manya's and Tanmoy's research-gap documents; full texts of
> survey papers 10 and 15; archive `uditdaftary/KGCR`; set `develop` as the default branch and
> protect `main`; tag `v1.0-Phase1` once `develop` merges to `main`.
...
**Scope: Udit (team lead) only.** The team is three students, but everything planned here is
authored by and attributed to the lead. Manya and Tanmoy get their branches created as scaffolding
...
|---|---|---|---|
| Abstract (200–300 words) | **Done** (298 words) | `docs/Project_Report.{md,docx}` | — |
...
| Research gap, papers 1–5 (lead) | **Done** | `docs/Research_Gap_Udit.{md,docx}` | Manya and Tanmoy write their own |
| 4–6 objectives | **Done** (6, each with a measure) | `docs/Objectives.{md,docx}` | — |
| Novelty summary (≤1 page) | **Done** | `docs/Novelty.{md,docx}` | — |
| Diagram 1 — AWS architecture | **Done** | `architecture/AWS_Architecture.{png,svg}` | Generated by `architecture/make_diagrams.py` from PMD §6 |
| Diagram 2 — system architecture | **Done** | `architecture/System_Architecture.{png,svg}` | Generated from the FD-01 S1→S10 flow; condition flowcharts kept as detail |
| Dataset details block | **Done** | `dataset/dataset_description.{md,docx}` | All mandated fields, corpus stated as synthetic |
| AWS services planning table | **Done** (21 services) | `docs/Project_Report.md` §8 | Planned vs running stated per service |
...
| `src/frontend/` | **Done** (stub) | `src/frontend/README.md` | Phase-II scope note |
| `results/` | **Exists — your strongest material** | `git mv artifacts/` + numbers | DF-7 12/12, ECE ≤0.05 vs 0.28 |
| `presentation/` | **Still missing** | deck | The one remaining content deliverable |
| `LICENSE` | **Done** (MIT) | root | — |
| main / develop / feature branches | **Done** | `feature/udit|manya|tanmoy` | — |
| 20–30 commits + ≥2 PRs (lead) | **Achievable from here** | `feature/udit` | See §8 |
...
git checkout -b feature/udit develop && git push -u origin feature/udit
```

Repeat for `manya`, `tanmoy` — created as scaffolding so they have somewhere to work; the lead
does not commit to them. Set `develop` as the GitHub default working branch; protect `main`.

**Step 5 — `CLAUDE.md` + `CHANGELOG.md`** on `feature/udit`, landed into `develop` by PR (see §5).
...
3. **Branch workflow** — `feature/<name>` → PR → review → `develop` → `main` → tag `v1.0-Phase1`.
...
4. **Allocation** — papers 1–5 / 6–10 / 11–15, research gap docs, contribution matrix rows, with the
   lead's scope (papers 1–5, `feature/udit`) marked as the only one being authored here.
...
3. **Research gap — `Research_Gap_Udit.docx` only**, own words, papers 1–5. Manya and Tanmoy
...
rendering `results/p4_df7_checkov_evidence.json` and the reliability diagram — gives Udit a real
...
- **Commit/PR attribution.** Scoped to the lead, the PDF's 20–30 commits and ≥2 PRs is reachable:
  ...
  commits on `feature/udit`, landing through real PRs into `develop`. What stays off the table
  is backdating or authoring commits under Manya and Tanmoy — their branches exist, their sections
  are theirs.
...
**Before the review:** `Research_Gap_Udit.docx` finalised, presentation deck, `LICENSE`, optional
frontend page, tag `v1.0-Phase1`. Manya and Tanmoy land their own gap docs on their own branches.
