...
analysis to reflect the student's own understanding, and these notes are a starting point, not a
substitute.
...
3. Nekrasov, R., Fossati, S., Kumara, I., Tamburri, D. A., & van den Heuvel, W.-J. (2026). IaC
   Generation with LLMs: An Error Taxonomy and A Study on Configuration Knowledge Injection.
   *ACM Transactions on Software Engineering and Methodology*. DOI: [10.1145/3817608](https://doi.org/10.1145/3817608)
4. Hu, H., Bu, Y., Wong, K., Sood, G., Smiley, K., & Rahman, A. (2023). Characterizing Static
   Analysis Alerts for Terraform Manifests: An Experience Report. *2023 IEEE Secure Development
   Conference (SecDev)*, 7–13. DOI: [10.1109/SecDev56634.2023.00014](https://doi.org/10.1109/SecDev56634.2023.00014)
5. Chu, Z., Wan, Y., Li, Q., Wu, Y., Zhang, H., Sui, Y., Xu, G., & Jin, H. (2024). Graph Neural
   Networks for Vulnerability Detection: A Counterfactual Explanation. *Proceedings of the 33rd
...
11. Jovanovski, D., Stojcheva, M., Dodevska, M., Lameski, P., Mishkovski, I., & Gjorgjevikj, D.
    ...
    *Information*, 17(4), 389. MDPI. DOI: [10.3390/info17040389](https://doi.org/10.3390/info17040389)
12. Jia, J., Yang, L., Wang, Y., & Sang, A. (2025). Hyper attack graph: Constructing a hypergraph
    for cyber threat intelligence analysis. *Computers & Security*, 149, 104194. Elsevier.
    DOI: [10.1016/j.cose.2024.104194](https://doi.org/10.1016/j.cose.2024.104194)
13. Morello, M., Sainio, P., & Alshawki, M. (2024). Regulatory Compliance Verification: A Privacy
    Preserving Approach. *2024 8th Cyber Security in Networking Conference (CSNet)*, 263–267. IEEE.
    DOI: [10.1109/CSNet64211.2024.10851761](https://doi.org/10.1109/CSNet64211.2024.10851761)
...
| # | Paper | Method | Dataset | Advantages | Limitations | Research gap |
|---|---|---|---|---|---|---|
| 11 | Jovanovski et al. (2026), *Information* (MDPI) | Privacy-preserving RAG that pairs LightRAG — a knowledge-graph retrieval system modelling typed edges for cross-references, semantic similarity and hierarchical dependencies — with locally hosted open-source LLMs | Curated benchmark of 222 multiple-choice questions with authoritative ground truth, built from official ISO/IEC 27000 standards and certification material | Directly addresses why chunk-based RAG fails on interconnected regulatory text; keeps data local, which matters for regulated organisations | Evaluated by multiple-choice question answering over standards text — it demonstrates retrieval of what a standard *says*, not application of a control to a concrete configuration | The knowledge graph models relationships *between clauses*. It never crosses into the configuration domain, so there is no edge from a clause to the resource property that satisfies it. That crosswalk is what turns compliance retrieval into compliance checking |
...
Read together, the fifteen papers separate into four groups, and the boundary between them is where
this project sits.

**Configuration security is measured, not reasoned about.** Verdet et al., Hu et al. and Vo et al.
all analyse Terraform through tools that evaluate one resource at a time. Hu et al. report that
resources with dependencies attract more alerts — the relational signal is visible in their data —
yet the analysis stays per-manifest. Detection quality improves (Wen & Ping reach 97% on real
misconfigurations) without the finding ever acquiring a justification an auditor could accept.

**Graphs are used reactively, on estates that already exist.** Elmiger et al. and Palma et al. both
demonstrate that graph structure exposes attack paths a per-entity review misses, which is the same
premise this project starts from. Both operate after deployment. Neither reasons at design time,
and neither binds a path to a regulatory clause.

**Compliance knowledge and configuration knowledge live in separate graphs.** Jovanovski et al.
model relationships between standard clauses; Elmiger et al. model relationships between cloud
entities. No paper in this set connects the two. That missing crosswalk is why a compliance system
can retrieve the clause but cannot say which resource property satisfies it.

**Intent is an input, never a recovered quantity.** Nekrasov et al. state the problem most directly:
knowledge injection lifts technical correctness sharply while intent alignment plateaus. Review
tools assume no intent at all and check against fixed rules. Recovering the intent an existing
estate was built to serve — and reporting how confident that recovery is — appears nowhere.

Some entries are adjacent rather than upstream, and should be presented that way: Jia et al. reason
over threat-intelligence prose, Guo et al. survey graph counterfactuals with no security domain,
Morello et al. verify a compliance claim that someone else derived. They belong in the survey
because they establish the techniques, not because they leave a hole shaped like this project.

---

## Status

- All 15 records verified against Crossref: title, authors, venue, year, volume, pages, DOI.
- Abstracts read for 13 of 15. Papers 10 and 15 (⚑) are metadata-verified only — full texts to be
  read via the university library and their cells completed before the review.
