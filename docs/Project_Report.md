...
| Persistence engine, L2/L3 run database & `kgcr history` (Manya) | Done | `src/backend/kgcr/persistence/`, 6 tests |
| Security context, INV-5/INV-7 policies & waiver governance (Manya) | Done | `src/backend/kgcr/auth/`, 5 tests |
| AWS adapter layer, fake adapter & boto3 STS assume-role (Manya) | Done | `src/backend/kgcr/aws_adapter/`, 4 tests |
| Backend service orchestrator & pipeline controller (Manya) | Done | `src/backend/kgcr/service/`, 3 tests |
...
**Test suite:** 148 tests total (146 passing, 2 optional-dependency skipped). Quality gates — `ruff check`, `ruff format --check`, `mypy`
